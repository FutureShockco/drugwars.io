	...
	<div class="border-bottom pt-2 pb-2 columns" :id="fight.transport_key.slice(0, 10)">
		<div class="columns text-center">
			<div class="column col-5">
				...
				<div v-if="fight.attacker_gang" class="gang-label mb-4 mt-1">{{ fight.attacker_role }} OF {{
					fight.attacker_gang }} [{{ fight.attacker_ticker }}]</div>
			...
			<div class="column col-2">
				...
				<h5 class="mt-0"
					v-else-if="fight.target_nickname === user.nickname && fight.attacker_territory === fight.target_territory && fight.attacker_base === fight.target_base">
					Units are back</h5>
			...
			<div class="column col-5">
				...
				<div v-if="fight.target_ticker" class="gang-label mb-4 mt-1">{{ fight.target_role }} of
					{{ fight.target_gang }}[{{ fight.target_ticker }}]</div>
		...
		<div>
			...
			<div class="text-center mb-3 mt-3">
				<div v-if="fight.attacker_base">
					FROM Territory {{ fight.attacker_territory }} : Location {{ fight.attacker_base }} - TO : Territory
					{{ fight.target_territory }} : Location {{ fight.target_base }}
				</div>
				<div v-if="fight.transporter_base">
					FROM Territory {{ fight.transporter_territory }} : Location {{ fight.transporter_base }} - TO :
				...
				<div v-if="fight.transport_key">
					Tx : {{ fight.transport_key }} <span v-if="fight.steem_block">Steem block :
						{{ fight.steem_block }}</span>
				</div>
				<div v-else-if="fight.transport_key">
					Tx: {{ fight.transport_key }} <span v-if="fight.steem_block">Steem block :
						{{ fight.steem_block }}</span>
				</div>
				<button class="button button-red mt-2" @click="handleSubmit()"
					v-if="fight.is_done && fight.attacker_nickname !== user.nickname || (fight.attacker_territory != fight.target_territory || fight.attacker_base != fight.target_base)">Cancel
					Station</button>
			</div>
		</div>
	</div>
