...
<template v-else>
    ...
    <img class="dwdicon" src="/img/icons/dwd.png"/>
     <span v-if="dwdPrice"> ${{ (dwdPrice * quantity) | amount }} =</span>
      {{ this.priceInDWD }} DWD
    </button>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        handleRequestPayment() {
            this.requestPayment({
                memo: `unit:${this.id},territory:${Number(this.base.territory)},base:${Number(this.base.base)},amount:${this.quantity}`,
