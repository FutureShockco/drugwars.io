...
const createParticles = function() {
  ...
  const pointFragmentShader = [
    ...
    `   float fogFactor = smoothstep(${parseInt(this.cameraDistance)}.0,${parseInt(
  ...
  const addTriangle = function(k, ax, ay, az, bx, by, bz, cx, cy, cz, lat, lng, color) {
    const p = k * 3;
    const i = p * 3;
...
function Globe(width, height, opts) {
  const baseSampleMultiplier = 0.7;
