 ...
 methods: {
  ...
  rotate: function(event) {
   if (!this.animate || this.count === 5) {
    ...
    var yval = (ypos / midpointX) * 30;
    var xval = (xpos / midpointY) * 30;
    var images = document.getElementsByClassName(this.card.hid + '-layer');
    c.style.transform =
  ...
  reset: function(event) {
   if (!this.animate || this.count === 5) {
    const self = this;
    const c = document.getElementById(this.card.hid + '-card');
    var images = document.getElementsByClassName(this.card.hid + '-card layer');
    ...
    for (var i = 1; i < images.length; ++i) {
     ...
     myImg.style =
      ...
      (myImg.getAttribute('data-depth') / myImg.clientHeight) * 2000 +
      'px); left: ' +
      (0 * myImg.getAttribute('data-depth') * -1) / 20 +
      '%; top: ' +
      (0 * myImg.getAttribute('data-depth')) / 20 +
      '%;';
    }
   }
  },
