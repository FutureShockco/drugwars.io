<template>
	<div class="d-flex flex-lg-row flex-column text-center text-lg-left item">
		...
		<div class="mx-auto" v-if="item.rewards && !ownJob">
			...
			<button :disabled="isLoading || inProgress || (item.type === 'gang' && !isBoss)" @click="handleSubmit" class="button btn-block button-green mb-2">
		...
		<div v-else>
			...
			<div class="columns m-2 p-2 text-center">
				...
				<div class="mb-2">
					...
					<i class="iconfont icon-clock mr-2" /> End in {{ timeToWait | ms }}
			...
			<router-link v-if="timeToWait && restUnits > 0" :to="`/actions?type=attack&target=${ownJob.territory}&base=${ownJob.base}&target_type=npc`">
			...
			<button v-else-if="!timeToWait" :disabled="(item.type === 'gang' && !isBoss)" @click="handleSubmit" :class="{ progress: waitingConfirmation }" class="button btn-block button-green mb-2">
			...
			<button v-else disabled :class="{ progress: waitingConfirmation }" class="button button-blue btn-block mb-2">
...
@import '../vars.less';
.shopcard {
	background: -webkit-gradient(linear, left top, left bottom, from(#0e111496), to(#000000cc)), url(/img/fake-brick.png);
