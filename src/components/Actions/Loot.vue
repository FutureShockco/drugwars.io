<template>
    <div v-if="stolenResources" class="mini" :class="{ 'text-red': result === 'lost', 'text-green': result === 'win' }">
        <div v-if="stolenResources.drugs && number">
            ...
            <span v-else>+</span> {{ stolenResources.drugs + ((stolenResources.drugs / 100 * 2.5) * number.length) | amount }}
            DRUGS
        </div>
        ...
        <div v-if="stolenResources.weapons && number">
            <span v-if="result === 'lost'">-</span>
            <span v-else>+</span> {{ stolenResources.weapons + ((stolenResources.weapons / 100 * 2.5) * number.length) |
            amount }} WEAPONS
        </div>
        ...
        <div v-if="(stolenResources.alcohols || stolenResources.alcohol) && number">
            <span v-if="result === 'lost'">-</span>
            <span v-else>+</span> {{ (stolenResources.alcohols + ((stolenResources.alcohols / 100 * 2.5) * number.length)) ||
                (stolenResources.alcohol + ((stolenResources.alcohol / 100 * 2.5) * number.length)) | amount }} ALCOHOL
        </div>
        ...
        <div v-if="stolenResources.dwd && number">
            <span v-if="result === 'lost'">-</span>
            <span v-else>+</span> {{ parseFloat(Number(stolenResources.dwd) +
                ((stolenResources.dwd / 100 * 2.5) * number.length)).toFixed(3) }} DWD
        </div>
