    ...
    <div class="d-flex flex-lg-row flex-column text-center text-lg-left item" :class="{ progress: inProgress, 'not-enough': hasNotEnough }">
        ...
        <div class="item-content width-full mr-3 mb-4">
            ...
            <div v-if="['drug_storage', 'weapon_storage', 'alcohol_storage'].includes(building.id)" class="mb-2">
                    <div v-if="ownItem.lvl"><b>Current capacity:</b> {{ 35000 * ownItem.lvl + (10000 + ((40000 * ownItem.lvl) / 100) * 10)*2.5 | amount }}</div>
                    <div v-if="ownItem.lvl"><b>Next capacity:</b> {{ 35000 * (ownItem.lvl+1) + (10000 + ((40000 * (ownItem.lvl+1)) / 100) * 10)*2.5 | amount }}</div>
                    <div v-else><b>Next capacity:</b> {{ 35000 * 1 + (10000 + ((40000 * 1) / 100) * 10)*2.5 | amount }}</div>
                    <div v-if="ownItem.lvl"><b>Safe:</b> {{ (35000 * ownItem.lvl + (10000 + ((40000 * ownItem.lvl) / 100) * 10))*2.5 /100*10 | amount }}</div>
                    <div v-if="ownItem.lvl"><b>Next Safe:</b> {{ (35000 * (ownItem.lvl+1) + (10000 + ((40000 * (ownItem.lvl+1)) / 100) * 10))*2.5 /100*10 | amount }}</div>
