		...
		<div class="columns text-center">
			<div class="column col-5">
				<div
					...
					:to="`/missions?type=attack?nickname=${fight.transporter_nickname}`">
					<Avatar :size="60" :username="fight.transporter_nickname" :picture="fight.transporter_picture"/>
				</div>
				<Avatar
					v-else
					:size="60"
					:username="fight.transporter_nickname"
					:picture="fight.transporter_picture"
				/>
				<div class="username mb-4">{{ fight.transporter_nickname }}</div>
				<div
					v-if="fight.transporter_gang"
					class="username gang mt-4"
				>{{fight.transporter_role}} OF {{ fight.transporter_gang }} [{{ fight.transporter_ticker}}]</div>
				<div
					v-if="json && json.transporter && json.transporter.value"
					v-html="json.transporter.value"
				></div>
			</div>
			<div class="column col-2">
				<h1 class="mt-0">TO</h1>
				<h5 class="mt-0" v-if="timeToWait">Delivery in <div>{{ timeToWait | ms }}</div></h5>
			...
			<div class="column col-5">
				<div
					...
					:to="`/missions?type=attack&nickname=${fight.target_nickname}`"
				>
