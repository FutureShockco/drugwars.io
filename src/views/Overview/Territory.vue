...
export default {
    ...
    computed: {
        main() {
            return this.$store.state.game.user.buildings.find(b => b.main === 1 && b.base !== 0 && b.territory !== 0) || null;
    },
    base() {
...
methods: {
        ...
        if (isValid && this.location && this.selectedTile) {
            ...
            this.send(payload)
                .then(() => {
                    ...
                    this.$router.push(`/map/territory?location=${this.location}`);
                })
                .catch(e => {
    ...
    async validateFormFree() {
        ...
        if (this.$store.state.game.user.buildings.find(b => b.main === 1)) {
