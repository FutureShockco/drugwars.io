<template>
        ...
        <div class="item-content width-full">
            <div v-for="level in nextLevels" :key="level" class="border-bottom mx-3">
                ...
                <div v-if="['drug_storage', 'weapon_storage', 'alcohol_storage'].includes(building.id)" class="mb-2">
                     <div v-if="level"><b>Safe:</b> {{ ( 10000+(35000 * level * (Math.sqrt(250-level)) / 100) * level ) /100*20 | amount }}</div>
                    <div v-else><b>Safe:</b> {{ 10000 /100*25 | amount }}</div>
                </div>
                <div v-if="['headquarters'].includes(building.id)" class="mb-2">
                  <div class="columns">
     <div class="column col-1">
    <div v-if="level"><b>Lvl 10</b> </div>
                    <div v-if="level"><b>Lvl 20</b> </div>
                    <div v-if="level"><b>Lvl 50</b> </div>
                    <div v-if="level"><b>Lvl 100</b></div>
    ...
    <div v-if="level"><b>Training Fac.</b> {{updateTime('training_facility',level,10) | ms}}</div>
                    <div v-if="level"><b>Training Fac.</b> {{updateTime('training_facility',level,20) | ms}}</div>
                    <div v-if="level"><b>Training Fac.</b> {{updateTime('training_facility',level,50) | ms}}</div>
                    <div v-if="level"><b>Training Fac.</b> {{updateTime('training_facility',level,100) | ms}}</div>
                    
                  </div>
                                                        <div class="column col-2">
                    <div v-if="level"><b>Crack House</b> {{updateTime('crackhouse',level,10) | ms}}</div>
                    <div v-if="level"><b>Crack House</b> {{updateTime('crackhouse',level,20) | ms}}</div>
                    <div v-if="level"><b>Crack House</b> {{updateTime('crackhouse',level,50) | ms}}</div>
                    <div v-if="level"><b>Crack House</b> {{updateTime('crackhouse',level,100) | ms}}</div>
                    
                  </div>
                  <div class="column col-2">
      <div v-if="level"><b>Pharma. Lab</b> {{updateTime('pharma_lab',level,10) | ms}}</div>
                    <div v-if="level"><b>Pharma. Lab</b> {{updateTime('pharma_lab',level,20) | ms}}</div>
                    <div v-if="level"><b>Pharma. Lab</b> {{updateTime('pharma_lab',level,50) | ms}}</div>
                    <div v-if="level"><b>Pharma. Lab</b> {{updateTime('pharma_lab',level,100) | ms}}</div>
                    
                  </div>
                  <div class="column col-2">
    <div v-if="level"><b>Research C.</b> {{updateTime('research_center',level,10) | ms}}</div>
                    <div v-if="level"><b>Research C.</b> {{updateTime('research_center',level,20) | ms}}</div>
                    <div v-if="level"><b>Research C.</b> {{updateTime('research_center',level,50) | ms}}</div>
                    <div v-if="level"><b>Research C.</b> {{updateTime('research_center',level,100) | ms}}</div>
                    
                  </div>
                    <div class="column col-2">
                    <div v-if="level"><b>Drug Storage</b> {{updateTime('drug_storage',level,10) | ms}}</div>
                    <div v-if="level"><b>Drug Storage</b> {{updateTime('drug_storage',level,20) | ms}}</div>
                    <div v-if="level"><b>Drug Storage</b> {{updateTime('drug_storage',level,50) | ms}}</div>
                    <div v-if="level"><b>Drug Storage</b> {{updateTime('drug_storage',level,100) | ms}}</div>
                ...
                <div v-if="['training_facility'].includes(building.id)" class="mb-2">
                 ...
                 <div v-if="['operation_center'].includes(building.id)" class="mb-2">
                    <div v-if="level"><b>Drug Bonus</b> +{{drugBonus(level) ||amount}} per day</div>
                    <div v-if="level"><b>Weapon Bonus</b> +{{weaponBonus(level) ||amount}} per day</div>
                    <div v-if="level"><b>Alcohol Bonus</b> +{{alcoholBonus(level) ||amount}} per day</div>
                </div>
                <div v-if="['research_center'].includes(building.id)" class="mb-2">
                   <div v-if="level"><b>Routing</b> {{updateTimeTrainings('routing',level,10) | ms}}</div>
                    <div v-if="level"><b>Close Combat</b> {{updateTimeTrainings('closecombat',level,20) | ms}}</div>
                    <div v-if="level"><b>FireArms</b> {{updateTimeTrainings('firearms',level,50) | ms}}</div>
                    <div v-if="level"><b>Protection</b> {{updateTimeTrainings('protection',level,100) | ms}}</div>
                     <div v-if="level"><b>Psychological Training</b> {{updateTimeTrainings('psychological',level,100) | ms}}</div>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    weaponBonus(lvl) {
      let oc = lvl;
      return parseFloat(
        this.HQ.weapon_production_rate * 60 * 60 * 24 * oc * 0.005).toFixed(2);
    },
    alcoholBonus(lvl) {
      let oc = lvl;
      return parseFloat(
